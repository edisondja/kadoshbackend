-- =====================================================
-- SCRIPT SQL SIMPLE - MIGRACIONES NUEVAS KADOSH
-- Fecha: 2026-01-16
...
-- 3. Si alguna tabla/columna ya existe, verás un error que puedes ignorar
--
-- =====================================================
...
-- Modificar columna dibujo_odontograma
ALTER TABLE `odontogramas` MODIFY COLUMN `dibujo_odontograma` longtext NOT NULL;
...
ALTER TABLE `citas` ADD COLUMN `google_event_id` varchar(255) DEFAULT NULL AFTER `fin`;

-- Agregar campo a procedimientos
ALTER TABLE `procedimientos` ADD COLUMN `comision` decimal(5,2) DEFAULT '0.00' AFTER `precio`;

-- Agregar campo a facturas
ALTER TABLE `facturas` ADD COLUMN `tipo_factura` varchar(255) DEFAULT 'servicio' AFTER `tipo_de_pago`;

-- Agregar campos a productos
ALTER TABLE `productos` ADD COLUMN `codigo` varchar(255) DEFAULT NULL AFTER `nombre`;
ALTER TABLE `productos` ADD COLUMN `precio` decimal(10,2) DEFAULT '0.00' AFTER `descripcion`;
ALTER TABLE `productos` ADD COLUMN `categoria` varchar(255) DEFAULT NULL AFTER `precio`;
ALTER TABLE `productos` ADD COLUMN `stock_minimo` int(11) DEFAULT '0' AFTER `cantidad`;
ALTER TABLE `productos` ADD COLUMN `activo` tinyint(1) DEFAULT '1' AFTER `stock_minimo`;

-- Agregar campo a doctors
ALTER TABLE `doctors` ADD COLUMN `especialidad` varchar(255) DEFAULT NULL AFTER `numero_telefono`;
